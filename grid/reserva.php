...
{
	$query  =  "SELECT top 20 CRV.NRORESERVA,CRV.OT,T.TDESCRI,
...
CRV.CENTROCOSTO ,CRV.TIPO,CRV.SOLICITANTE FROM
 [022BDCOMUN].DBO.RESERVA_CAB AS CRV
INNER JOIN [010BDCOMUN].DBO.TABAYU AS T ON CRV.SOLICITANTE=T.TCLAVE WHERE TCOD='12' AND CRV.ESTADO='00'  AND CRV.FECHA BETWEEN 
 '$fecha' AND '$fecha'  ORDER BY CRV.NRORESERVA DESC";
}
else
{
...
CRV.CENTROCOSTO ,CRV.TIPO,CRV.SOLICITANTE FROM
 [022BDCOMUN].DBO.RESERVA_CAB AS CRV
INNER JOIN [010BDCOMUN].DBO.TABAYU AS T ON CRV.SOLICITANTE=T.TCLAVE WHERE  TCOD='12' AND 
CRV.ESTADO='00'
